      ...
      <view slot="back" class='tn-custom-nav-bar__back'
		...
		<view class="top-backgroup">
			...
			<view class="nav_title--wrap tn-margin-bottom-sm">
				<view class="nav_title tn-cool-bg-color-15">{{ category.title}}</view>
			</view>
			<view class='nav-list'>
		...
		<view class="tn-padding-bottom-xs"></view>
		 ...
		 <tn-popup v-model="show" mode="center" borderRadius="10" width="50%" height="auto" closeBtn="true" marginTop="10">
		      <view style="margin: 30rpx;"><rich-text :nodes="sourceContent.content"></rich-text></view>
		</tn-popup>
...
<script>
	import { itemList, item } from '@/utils/api/source'
	...
	export default {
		...
		onLoad(params) {
			let category = JSON.parse(params.params)
			this.category = category
			this.searchWhere.category_uid = category.uid
			this.itemList()
		},
		methods: {
			clickMenu(obj) {
				item({uid: obj.uid}).then(res => {
					this.sourceContent = res
					this.show = true
				})
			},
			itemList() {
				itemList(this.searchWhere).then(res => {
